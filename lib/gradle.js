        ...
        fs_1.access(path_1.join(cwd, path_1.join(pd, "gradlew")), fs_1.constants.F_OK, err => {
            if (err) {
                ...
                if (process.platform === 'win32') {
                    resolve(path_1.join(pd, "gradlew.bat"));
                }
                else {
...
function getTaskToPublish(cwd, pd, t, env, logger) {
    return new Promise(async (resolve, reject) => {
        ...
        const child = child_process_1.spawn(command, ["-p " + pd, "tasks --all", "-q"], {
            cwd,
            env,
...
function getVersion(cwd, pd, env) {
    return new Promise(async (resolve, reject) => {
        const command = await getCommand(cwd, pd);
        const child = child_process_1.spawn(command, ["-p " + pd, "properties", "-q"], {
            cwd,
            env,
            shell: process.platform === 'win32',
            stdio: ["inherit", "pipe"],
...
function publishArtifact(cwd, pd, t, env, logger) {
    return new Promise(async (resolve, reject) => {
        ...
        const child = child_process_1.spawn(await command, ["-p " + pd, await task, "-q"], { cwd, env, shell: process.platform === 'win32' });
        child.on("close", code => {
            if (code !== 0) {
